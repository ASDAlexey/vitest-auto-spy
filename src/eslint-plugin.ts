 ...
 * itself, so it decides on a heuristic.
 *
 * **`no-ts-expect-error-on-double` and `no-constant-expect` decide on facts** — a suppression over a double's
 * configuration, a value the spec spelled out — so they are `error`. **`no-compile-components` is `error` and
...
const recommendedRules: Record<string, RuleSeverity> = {
  ...
  [`${PLUGIN_NAME}/no-vacuous-absence-assertion`]: 'error',
  ...
  [`${PLUGIN_NAME}/no-reflect-member-access`]: 'error',
  ...
  [`${PLUGIN_NAME}/prefer-settle-dynamic-import`]: 'error',
  [`${PLUGIN_NAME}/prefer-create-mock`]: 'warn',
  [`${PLUGIN_NAME}/no-mock-cast`]: 'error',
  ...
  [`${PLUGIN_NAME}/no-unknown-use-value-key`]: 'error',
  ...
  [`${PLUGIN_NAME}/no-self-called-spy`]: 'error',
