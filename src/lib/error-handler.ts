...
const actualArgumentsMessage = (actualArgs: unknown[]): string => {
  // `stringify` wraps the (always non-empty) array in brackets — drop them to
  // show a bare arg list. `String(...)` keeps the result total against its
  // `string | undefined` signature without an unreachable fallback branch.
  const formatted = String(stringify(actualArgs));
