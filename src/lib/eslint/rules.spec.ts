...
describe('no-dead-schemas', () => {
  ...
  it('flags schemas on a module that declares nothing', () => {
    expect(lint(bed('imports: [FooterComponent], schemas: [NO_ERRORS_SCHEMA]'), RULE)).toEqual(['vitest-auto-spy/no-dead-schemas']);
  });

  it('reports the `schemas` entry itself, so the fix is the reported line', () => {
    const [message] = verify(bed('imports: [C], schemas: [NO_ERRORS_SCHEMA]'), RULE);
  ...
  it('flags a configuration with nothing but schemas in it', () => {
    expect(lint(bed('schemas: [NO_ERRORS_SCHEMA]'), RULE)).toHaveLength(1);
  });

  it('leaves a live schema alone', () => {
    expect(lint(bed('declarations: [HostComponent], schemas: [NO_ERRORS_SCHEMA]'), RULE)).toEqual([]);
  });
  ...
  it('leaves a schema alone when another configuration in the file declares something', () => {
    ...
    expect(lint(split, RULE)).toEqual([]);
  });
  ...
  it('says nothing about an empty list, or one it cannot count', () => {
    ...
    expect(lint(bed('imports: [C], schemas: []'), RULE)).toEqual([]);
    expect(lint(bed('schemas: [NO_ERRORS_SCHEMA], declarations: DECLARATIONS'), RULE)).toEqual([]);
    expect(lint(bed('schemas: [NO_ERRORS_SCHEMA], declarations: [...SHARED]'), RULE)).toEqual([]);
  });

  it('reads only `TestBed.configureTestingModule` with a literal configuration', () => {
    expect(lint('bed.configureTestingModule({ schemas: [NO_ERRORS_SCHEMA] });', RULE)).toEqual([]);
    expect(lint('TestBed.configureTestingModule(moduleDef);', RULE)).toEqual([]);
    expect(lint('TestBed.overrideModule(M, { set: { schemas: [NO_ERRORS_SCHEMA] } });', RULE)).toEqual([]);
  });
  ...
  it('still reports the module-level schema of a file that also overrides a component', () => {
    ...
    expect(lines(both, RULE)).toEqual([1]);
  });
  ...
  it('reports every dead configuration of a file, not the first', () => {
    ...
    expect(lines(two, RULE)).toEqual([1, 2]);
  });
});
...
describe('no-done-callback — done.fail', () => {
  ...
  it('reports done.fail on top of the parameter, because the two fail differently', () => {
    ...
    expect(lint(code, RULE)).toHaveLength(2);
    // The parameter report explains the missing callback; this one explains the green run.
    expect(verify(code, RULE)[1]?.message).toContain('done.fail is not a function');
    expect(verify(code, RULE)[1]?.message).toContain('rejects.toMatchObject');
  });
