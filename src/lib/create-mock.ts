 ...
 * `eslint-disable` at a time) and a named helper whose signature keeps `Partial<T>` checked.
 * Fields that do not exist on `T`, or exist with a different type, are still rejected.
 *
 ...
 * ```
 *
 * @param partial The fields to populate, checked against `T`. Everything else is `undefined` at
 *   runtime while the static type stays `T` — so an assertion on an unseeded field is a bug in the
 *   test, not in the helper.
 */
