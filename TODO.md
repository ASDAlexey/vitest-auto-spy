...
- [ ] **`expectEmission` cannot subscribe to an Angular `output()`.** `SubscribableLike`
      (`lib/expect-emission.ts:23-25`) requires an *observer object*, and the helper passes one at
...
- [ ] **`bench/auto-spy.bench.ts` measures the garbage collector.** `@vitest/spy` holds every mock
      ...
      **272.67× faster** than lazy" for the 40-method case the docs publish as a 7× *lazy* win. Fix:
      ...
      published).
- [ ] **The declaration output emits a *value* import of rxjs.** `dist/types-*.d.ts:1` is
...
- [ ] **Pre-serialise `calledWith` config args at `set()` time.** `ArgsMap.#valueMatches`
      re-serialises the *config* arg on every call, and the config arg never changes: one asymmetric
...
- [ ] **Opt-in `lazySpies: 'proxy'` — one `Proxy` instead of N accessor placeholders.** Creation is
      ...
      **Must stay opt-in** — the Proxy cannot remove itself, so it imposes a permanent +25.3 ns per
      read (4.09 → 29.43 ns) and +29.5 ns per call, and at width 5 with methods touched it *loses*
...
- [~] **Micro-optimising `createFunctionSpy`**, re-confirmed with fresh numbers rather than quoted
      ...
      first use was right.
- [~] **`lazySpies: 'proxy'` as the *default*.** Loses 158 B at width 5 with methods touched and
...
- [ ] **`stable(fixture, { timeout = 2000, label })` — S.** Race `fixture.whenStable()` against a
      ...
      runner report a 5 s file-level timeout: *"the fixture was still unstable after 2000 ms. A
      pending HttpClient request keeps it unstable, and under `provideHttpClientTesting` only the
      spec can complete one — flush it before awaiting, or use `settleResource()`."* This is the
...
project that merely *has* this package installed writes tests with it, correctly, with no human
configuration.
...
package ships in its tarball is never auto-discovered. Two channels *do* work with no setup at all,
...
- [ ] **`npx vitest-auto-spy init`, on the same `bin` as `doctor`.** That shared entry is what the
      ...
      `require()`, and `init` writes files rather than creating spies).
      - **Tier 1, always written:** a managed block in root `AGENTS.md` and root `CLAUDE.md`, plus
        ...
        stub's frontmatter against the shipped skill, the same CI pattern as `llms:check`.
      - **Tier 2, only when the tool's directory already exists**, glob-scoped so it costs no context
        ...
        are pointers, not copies — all of them read `AGENTS.md` anyway.
      - **Never create `.rules`, `.cursorrules`, `.windsurfrules` or `.clinerules`** (the legacy file
        ...
        Append only if one already exists.
      - **Markers and idempotency:** `<!-- vitest-auto-spy:begin v=… sha=… -->` / `:end`, content
        ...
        record every touched path in a sidecar manifest so an `--uninstall` can be exact.
      - **The reason a CLI beats the paste-able snippet the README ships today:** `init` reads the
        ...
        states all of it and half of it is false for any given repo.
      - **Codex budget:** the whole root→cwd `AGENTS.md` chain is capped at `project_doc_max_bytes`,
        default **32 768 bytes**, and over-budget files are silently truncated. Target ≤1.6 kB for
        the block and warn when the resulting file crosses the cap. Nested `AGENTS.md` *below* cwd
...
- [~] **A postinstall message.** Ineffective and risky, and the trend is one-way: npm hides
      ...
      refuses to minify. Instead: first-run detection *inside* the CLI ("no agent instructions found
...
- [~] **Shipping an MCP server.** Every connected server costs context in **every** session; three
      ...
      registration is *more* setup than `init`, not less (Claude Code prompts on first use, and a
...
- **`no-unawaited-helper`** — an unawaited `expectEmission` / `stable` / `flushEventLoop`. The proof
  that a doctor can beat a type-free linter *without* a type checker: the callee is resolved through
...
- **`ts-auto-mock`** is feature-frozen *by its author* and explicitly does not work with esbuild or
...
prototype walk — and it is *worse* in two ways (no getter handling at all, and the recursion has no
...
- **Accessor spies on Bun.** `bun:test`'s `spyOn(obj, 'prop', 'get')` throws *"does not support
  accessor properties yet"*; `lib/redefine-accessor-spy.ts` never calls it. Nobody else can do this
...
- **`injectSpy` warns when the injector returned the real thing** (`warnWhenNotASpy`,
  `lib/angular.ts`). Spectator types *every* token as a spy, mocked or not.
...
- [ ] **`vi.defineHelper` on every throw site — S.** Vitest 4.1's `defineHelper(fn)` exists so a
      wrapper's failure points at the *caller's* line. This package throws from five places a user
...
- [ ] **`strict: true` / `onUnstubbedCall` — S.** Throw, naming class, method and args, when a method
      ...
      `onlyMethodsToSpyOn`, which *deletes* the method, so the failure reads `service.load is not a
...
- [ ] **`createSpyFromInstance(instance, config)` — M.** The one structural capability three live
      ...
      `td.replace(obj, 'method')`. Every factory here *constructs* the double; there is no way to
...
- [ ] **A `/migrating` page aimed at `refactor-jasmine-vitest` output.** Show the codemod's
      ...
      TODO cases explicitly — this library handles them *by construction*, because it reads the
