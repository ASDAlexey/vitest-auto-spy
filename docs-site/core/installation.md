...
`vitest-auto-spy/angular/matchers` since 6.0, on `/angular` before it — import `By` from it as a
...
| `vitest-auto-spy/angular`             | `provideAutoSpy`, `injectSpy`, `renderShallow`, `createWithAutoSpies`, `stable`/`flushEffects`, the `mock*Prop` helpers — the matchers, diagnostics and doubles below left this entry in 6.0                                                                            | `@angular/core`                                          |
| `vitest-auto-spy/angular/diagnostics` | `enableAngularDiagnostics` and the whole TestBed timing family. A companion to `/angular`, not a second copy of the core — moved off it in 6.0                                                                                                                          | `@angular/core`                                          |
| `vitest-auto-spy/angular/doubles`     | The Material dialog trio and the `Window`/`Document` platform doubles; registers the Vitest adapter, so its doubles spy out of the box. A companion to `/angular`, moved off it in 6.0                                                                                  | `@angular/core`                                          |
| `vitest-auto-spy/angular/matchers`    | `registerDirectiveMatchers`, `registerResourceMatchers`, `registerSignalMatchers` — one `expect.extend` each, from the setup file. A companion to `/angular`, moved off it in 6.0                                                                                       | `@angular/core`, `@angular/platform-browser`             |
