...
also carries the report, and last for token registrations, +18 B. The dispatch-replacement report
then rode every runtime entry, +0.28…0.44 kB (`/setup` +85 B), taking the root entry from 17.19 to
17.50 kB — its hook and message text live in the shared spy engine — with `/angular`, `/bun` and
`/node` adding the `renderShallow` shape fix on top.
...
exempts; it is a lint-time entry point that no spec run loads. `prefer-provide-auto-spy` and
...
26.9 → 30.4 kB, `/setup` 17.1 → 18.9 kB — and +0.1…1.3 kB on the chunked ones.

Two releases then moved the table to what it says now. v5.20.0 put +1.74…1.79 kB on every row that
carries the core — `mockDeep` arrays and `fallbackMockImplementation`, `createSpyFromInstance`
passthrough and its only-list rules, `adoptMock` and `moduleNamespace` passthrough — taking the root
...
The published tarball is **1 019 kB** over **114 files** (`npm pack --dry-run`, 2026-09-26) against
