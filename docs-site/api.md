...
`extendWithAutoSpies`, `ComponentStubOptions` for `createComponentStub`, and `MatDialogRefInit<R>`,
`MatDialogRefDouble<Ref>`, `DialogRefLike` and `DialogResult<Ref>` for the dialog doubles; `/dom-stubs` adds
