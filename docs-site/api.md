...
never needs two imports from this package. `/angular-http` is the one deliberate exception: it is a
companion to `/angular` rather than a replacement for it, and staying narrow is what keeps
`@angular/common` confined to the suites that ask for it.
