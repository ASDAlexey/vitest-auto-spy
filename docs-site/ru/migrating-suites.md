...
Оба слова стоит сохранить и за пределами Nest. `TestBed` в Angular делает тот же выбор по одному
провайдеру за раз: `provideAutoSpy(Dep)` для каждого коллаборатора — это solitary-спека; оставить один
настоящим — не указав его или указав настоящий класс — делает её sociable, и шов сдвигается ровно
настолько, насколько сказала спека. Билдер учить не нужно: список провайдеров уже им является.
...
Конфиг Vitest тоже не меняется. esbuild, а значит и Vite, не выдаёт `design:paramtypes`, поэтому
Nest-сюита на Vitest уже прогоняет спеки через SWC (`unplugin-swc`). `createNestUnit` читает те же
метаданные, так что плагин остаётся: это единственная часть стоимости настройки Suites, которая на
самом деле принадлежит Nest и никуда не уходит.

Один файл можно удалить: тот `global.d.ts`, который ссылается на `@suites/doubles.vitest/unit`, чтобы
дополнить `Mocked<T>` и `unitRef.get()` типами моков Vitest. Он существует потому, что собственный
путь адаптера к тому же результату — `postinstall`-скрипт, который дописывает эту ссылку в начало
`dist/esm/index.d.ts` и `dist/cjs/index.d.ts` пакета `@suites/unit` внутри `node_modules`, — ничего не
делает там, где install-скрипты зависимостей не запускаются (pnpm 10 по умолчанию, `--ignore-scripts`),
или там, где два пакета установлены не рядом. `Spy<T>` — обычный экспортируемый тип: дополнять нечего
и патчить нечего.
...
- **Остальная часть вашей сюиты на том же ядре.** [Angular](/ru/adapters/angular) — чего Suites не
  ...
  `readonly #x = inject(X)` таких метаданных не выдаёт; Angular-адаптера у него нет, как нет и
  DI-адаптеров, кроме Nest и Inversify, — плюс [React](/ru/adapters/react), [Vue](/ru/adapters/vue) и
  [Svelte](/ru/adapters/svelte), из одной зависимости.
- **Потоки и аксессоры.** Дубль Suites — это `Proxy` над `{}`, отвечающий на любое имя через
  `vi.fn()`, поэтому метод, возвращающий `Observable`, не получает хелперов для потоков, а геттер не
  сохраняется вовсе — его чтение возвращает мок-функцию. Здесь метод, возвращающий `Observable`,
  получает [`nextWith` и остальные](/ru/core/control-helpers#observable-methods-properties-—-nextwith),
  а геттеры и сеттеры — [собственные спаи](/ru/core/create-spy-from-class#accessor-spies-—-accessorspies).
...
выходило ничего; релизы 3.1.x, появившиеся позже, — это ветка 3.x. До них ничего не публиковалось
между 3.0.1 от 2025-01-02 и `4.0.0-alpha.0` от 2025-10-27. Всё на этой странице прочитано из
опубликованных тарболов, а не с сайта документации, а версии и даты перечитаны из реестра 2026-09-19 —
без изменений, `latest` по-прежнему 3.1.1. За неделю по 2026-09-18 `@suites/unit` скачали 80 249 раз,
`@suites/doubles.vitest` — 25 353.
