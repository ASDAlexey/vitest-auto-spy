...
Рецепты под конкретную задачу живут на отдельных страницах: [моки классов](/ru/guides/mocking-classes),
[мок `localStorage`](/ru/guides/mocking-local-storage), [мок Prisma Client](/ru/guides/mocking-prisma)
и [стори Storybook с авто-спаями](/ru/guides/storybook-angular).
...
Некоторые аргументы не имеют литерала, с которым их можно сверить: колбэк, который собрал сам
тестируемый код, объект опций, склеенный из трёх источников, `FormData`. `captureArg` перехватывает
такой аргумент — там, где ожиданию иначе пришлось бы его описывать.
...
`.values` хранит все значения, которые захватчику **предложили**, а это не то же самое, что все
совпавшие. Раннер пробует ассерт на каждой записанной записи о вызове и сравнивает позиции слева
направо, останавливаясь на первой, которая расходится; захватчик совпадает со всем, поэтому тот,
что стоит левее позиции, отвергающей вызов, уже успел записать аргумент этого вызова. Захватчику в
последней позиции предлагают только вызовы, сошедшиеся по всему, что до него.

Если список обязан означать «совпавшие», дайте захватчику фильтр. Тогда соответствие решает он сам
и записывает только то, что принимает:
...
`captured` говорит, что захватчику что-то предложили, а не что ассерт прошёл, — так что проверяйте
само ожидание, а захватчик читайте ради того, что вызов нёс.
