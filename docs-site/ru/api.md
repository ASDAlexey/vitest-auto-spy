...
| `vitest-auto-spy/eslint-plugin`                                                                              | Двадцать пять правил линтера для flat-конфига, которые направляют сюиту на эти хелперы                                                                                                             |
...
| `/setup`            | `setupAutoSpy`, `setupFakeTimers`, `advanceTimers`, `mockSystemTime`, `withSystemTime`, `mockNow`, `useCountingClock`, `registerFocusMatchers`, `blockNetwork`, `guardGlobalPatches`, `guardPrototypePollution`, `installPerTest`, `restoreTimerGlobals`, `restoreWebStorage`, `setSpyEngine` / `getSpyEngine` / `SpyEngine`, трекеры зависших таймеров и реджектов, трекеры реестра моков, `describeDuplicateCopies`, `getPackageCopies`                                                                                                                                 |
