...
features:
  ...
  - title: Хелперы, которые следуют за типом возврата
    details: 'Метод, возвращающий Promise, получает resolveWith и rejectWith, возвращающий Observable — nextWith и throwWith, а каждый метод получает calledWith, mustBeCalledWith и failWith. При чтении значения обратно narrow.defined возвращает его без null и undefined прямо внутри выражения, которому оно нужно; toBeDefined() в Vitest 5.0 ничего не сужает, а assert.exists сужает, но ничего не возвращает, и каждое необязательное чтение стоит лишней инструкции и локальной переменной. adoptMock даёт те же хелперы vi.fn(), который уже собрала фабрика vi.mock, и сохраняет записанные им вызовы. createLog журналирует порядок вызовов между коллаборантами — там, где toHaveBeenCalled проходит в любом порядке; это класс, который сам Angular держит в трёх копиях, отгруженный один раз и без привязки к раннеру. expectAllEmissions отдаёт все значения потока, когда он завершился, с типом из потока, а expectEmissions(source$, n) останавливается на n и следующего уже не видит. outOfType типизирует заведомо неверную фикстуру — null, который бэкенд присылает там, где тип обещает объект, — как T прямо в месте вызова, без каста, на который среагирует правило линтера. stubWorker даёт jsdom и happy-dom, которые не исполняют скрипт воркера, Worker, чьим скриптом служит сама спека; слушатели складываются, ответ приходит микрозадачей, а сообщения проходят через structuredClone, так что колбэк, отправленный по ошибке, падает с тем же DataCloneError, что и в браузере. stubAnimationFrame отдаёт requestAnimationFrame спеке — в режиме immediate колбэк выполняется до возврата вызова, в режиме queued каждый кадр ждёт flush(), так что цикл анимации продвигается на шаг за flush, а не уходит в рекурсию; stubElementRect заставляет getBoundingClientRect() вернуть настоящий DOMRect, тогда как jsdom для любого элемента отвечает нулями.'
  ...
  - title: Vitest 5 тем же пакетом
    details: 'Один пакет покрывает Vitest с 2.1 по 5.x — без второго мажора, без раздвоенных типов, без единой правки в спеке. Та же сюита идёт на 7.7 % быстрее на Vitest 5, а встроенный движок спаев даёт ещё 8.1 % поверх vi.fn(). На Angular 22.2, чей unit-test-билдер первым работает на Vitest 5, сюита из 700 файлов с покрытием v8 проходит за 8.91 с вместо 16.50 с — на 46 % меньше времени, в 1.85 раза быстрее.'
...
<div class="vas-fact"><b>49</b><span>правил линтера</span></div>
