...
| --------------------------------- | ------- | ---------------------------------------------------------------------------------------------------------------- |
| `jasmine-namespace-without-entry` | `warn`  | `.and` / `.calls` / `.withArgs` на спае библиотеки в файле, который нигде не ставит слой                        |
...
| `prefer-native-spy-api`           | `off`   | `.and` / `.calls` там, где собственный API спая говорит то же самое — **`--fix`**, если получается проследить получателя |
