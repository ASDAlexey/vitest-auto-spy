...
`@angular/core` на 2026-09-26: `latest` — **22.2.0**, `v21-lts` — **21.2.24**, `v20-lts` —
**20.3.32**):

- **`@angular/build:unit-test` помечен `[EXPERIMENTAL]` вплоть до 22.1.x** и теряет эту пометку в
  **22.2.0** (angular-cli PR #34095). Работать пометка ему никак не мешает; но на более старой версии
  она означает, что опции билдера не попадают под политику депрекации Angular.
