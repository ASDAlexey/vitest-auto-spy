...
`@analogjs/vite-plugin-angular` плюс файл настройки TestBed. На `@angular/build` 22.2 пара пакетов
Analog — `@analogjs/vite-plugin-angular` и `@analogjs/vitest-angular` — должна быть **2.7.5 или
новее**; более старые падают на старте с `TypeError: cache.has is not a function`, и
`npx vitest-auto-spy doctor` сообщает об этом как
[`analog-behind-angular-build`](/ru/utilities/cli#analog-behind-angular-build).

::: warning Что компилирует unit-test-билдер начиная с 22.2.0
Начиная с `@angular/build` 22.2.0 программа `@angular/build:unit-test` собирается из файлов спек,
`providersFile`, `setupFiles` и тех `.d.ts`, которые включает `tsconfig.spec.json`, — плюс всё, что
они импортируют. Обычный `.ts`, который только перечислен в этом `include`, — например, с
`import 'vitest-auto-spy/rxjs'` или с аугментацией `declare module`, — в неё больше не входит, и
всё, что он объявлял, пропадает из каждой спеки. Перенесите его в файл настройки, в спеку или в
`.d.ts` — с `import` или `export {}`, чтобы аугментация осталась аугментацией. Более ранние билдеры компилируют весь `include`.
:::
...
`keepTemplate: true` сохраняет словарь шаблона, читая импорты, которые компилятор записал в `ɵcmp`, а
приходят они в трёх видах: плоским массивом, фабрикой, которая его возвращает, или `null` у
компонента, который ничего не импортирует. Это не вопрос JIT против AOT — JIT всегда пишет фабрику, а
AOT пишет массив, если только **цикл** между двумя компонентами не заставит отложить чтение.
Поддержаны все три; безусловный вызов массива давал `TypeError: dependencies is not a function` на
...
разрешимым. AOT — нет: ngtsc разрешает модуль во время компиляции и расплющивает его экспортируемые
объявления в этот список, так что приходит `standalone: false`-пайп или директива без модуля рядом —
а Angular принимает в `imports` только standalone-объявления и `NgModule`. Если оставить это
Angular, отказ выглядит как
`The "WhisperPipe" pipe, imported from "ReportComponent", is not standalone. Does the pipe have the standalone: false flag?`,
и читается как указание пойти и поправить этот пайп; с пайпом всё в порядке, и под JIT этот же вызов
работает. `renderShallow` проверяет скоуп, который собирается отдать, и бросает раньше — называя
объявления и говоря, что скоуп нельзя восстановить из того, что хранит определение. Дальше два пути:
...
Фикстура, чей `componentType` не несёт `ɵcmp`, тоже отклоняется по имени: называется и класс, и то,
чем он должен был бы быть, чтобы входы вообще были, — `@Directive` получает входы через хост-элемент,
который его применяет, у `@Pipe` их нет, а класс, который Angular не компилировал, не несёт
определения вовсе. Раньше определение читалось прямо в `Object.entries` и падало как
`Cannot read properties of undefined (reading 'inputs')` — без компонента и без починки в тексте;
`createComponentStub` отвечает на тот же вопрос по-человечески с самого начала, и теперь эти два
читаются одинаково.
...
В названии стоит zoneless, и хелперы написаны под него, но сами по себе они не только для него:
`stable` и `setInputs` — те же две строки в сюите на зонах, и с 5.9.0 именно на них такую сюиту и
переводят. Поэтому тик выполняется внутри `TestBed.inject(NgZone)`: в zoneless это `NoopNgZone`, чей
`run` — прямой вызов, и он ничего не стоит.

Под zone.js это разница между «работает» и «нет». `TestBed.createComponent` создаёт компонент внутри
`ngZone.run(…)`, поэтому каждый `effect()`, зарегистрированный его конструктором, запоминает
внутреннюю зону `NgZone` как свою. Тик, начатый из тела теста, идёт в зоне раннера, так что
`runEffectsInView` прыгает обратно — `effect.zone.run(() => effect.run())` — чтобы прогнать **грязный**
эффект, а выход из этого прыжка переводит зону из нестабильной в стабильную. Об этом сообщает
`NgZone.onMicrotaskEmpty`, а подписчик, которого ставит `provideZoneChangeDetection()`, отвечает на
него вызовом `ApplicationRef._tick()` — защищённым от собственного планировщика, но не от
`ApplicationRef._runningTick`. Тик, уже стоящий на стеке, входит повторно, и Angular бросает
`NG0101: ApplicationRef.tick is called recursively`.

Три вещи делают это сначала труднодостижимым, а потом незаметным. Все составляющие обычные — Angular
CLI в `@angular/build:unit-test` добавляет `provideZoneChangeDetection()` в любую сюиту, которая
грузит zone.js, а компонент с одним `effect()` не экзотика, — но эффект должен быть **грязным** именно
в этот момент, поэтому один и тот же вызов может работать сколько угодно и упасть в первый раз, когда
он ведёт первый рендер фикстуры. А ошибка уходит в `ErrorHandler`, а не бросается на месте вызова, так
что сюита, которая не падает на выводе в консоль, остаётся зелёной с недоделанной детекцией изменений.

Замерено на сюите Angular 22 из 1771 спеки: переписывание 451 вызова `componentRef.setInput` в
`setInputs` сделало красными **57 зелёных файлов**, все на `NG0101`, и ни один из них не красный после
того, как тик переехал внутрь зоны.

Одно поведение для сюиты на зонах действительно меняется: выход из зоны объявляет её стабильной, и тот
же подписчик тикает ещё раз. По счётчику `ApplicationRef.afterTick` один `flushEffects()` — это 1 → 2
тика приложения под zone.js и столько же, сколько было, под zoneless: тот же второй проход, который в
этом месте всегда делал `fixture.detectChanges()`, и он не находит ничего грязного.
...
`flushEffects()` из этого пакета — это `TestBed.tick()`: нижняя граница здесь Angular 20, поэтому
отката к `ApplicationRef.tick()` больше нет, и сюита, которая им пользуется, уже сидит на выжившем
вызове.
...
(`@angular/build/src/tools/esbuild/utils.js:166` в 22.2.0) — сознательный обход дефекта производительности
...
которая обсуждается ниже, против этой фикстуры не прогонялась вовсе: на момент замера её ещё не
существовало.
...
закрывает окно: обновитесь до 22.1.7 или новее и уберите `"splitting": false` с тестового таргета,
если он там есть, — исправление и есть значение по умолчанию. Начиная с 22.2.0 опция помечена
устаревшей («No longer needed with Vitest 5»), по умолчанию по-прежнему `true`, так что выставлять
ничего не нужно.
...
поскольку уведомление держит флаг на `globalThis`, который глушит каждое следующее вычисление, —
называя версию, оба выхода и способ отключения. Ради
...
На 22.1.5 и 22.1.6, если не обновляться, единственный рычаг — пропатчить установленный билдер на месте. Форма такой заплаты —
...
пакета, весь смысл которого в том, что падения сами называют свою причину. И прожила она недели:
22.1.7 закрыл окно, ради которого она существовала.
...
::: tip На 22.2.0 Vitest 5 сокращает прогон с покрытием на треть–половину
Начиная с `@angular/build` 22.2.0 билдер работает на Vitest 5, и именно покрытие от этого дешевеет. На
zoneless-сюите Angular 22.2 из 700 файлов и 11 491 теста, использующей эту библиотеку, с настройками
билдера по умолчанию переход раннера с Vitest 4.1.11 на 5.0.2 сократил `ng test --coverage` с 16,50 с до
8,91 с на v8 (**−46 %**) и с 37,07 с до 23,92 с на istanbul (**−35,5 %**); без покрытия обе версии идут
вровень. Для обновления нужны `vitest` и `@vitest/coverage-*` 5, поднятые вместе, а на Analog — 2.7.5
или новее; `overrides` не нужны. Билдеры постарше продолжают работать на Vitest 4. Таблица, методика и
оговорки —
...
возможен, он дешевле обёртки, — а под `@angular/build:unit-test` он возможен начиная с 22.2.0, чей
peer-диапазон допускает Vitest 5 (до 22.2 он заканчивался на `^4`); `overrides` не нужны.
Пользователям Analog для того же нужна версия 2.7.5 или новее.
:::
...
Способ починки зависит от того, в чём укоренена фикстура. На фикстуре тестируемого компонента
директиве место в его `hostDirectives` или `imports`, и падение говорит это, называя компонент;
хост, собранный `createDirectiveHost` или `TestBed.createDirective`, существует только в спеке, так
что там падение по-прежнему отправляет к `createDirectiveHost({ template, scope })`.
