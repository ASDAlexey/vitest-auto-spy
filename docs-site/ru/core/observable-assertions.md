...
Написанную руками форму третьей строки — `let`, который заполняет колбэк `subscribe`, проверенный
через `toEqual([])` против собственного инициализатора, — ловит правило
[`no-vacuous-absence-assertion`](/ru/utilities/eslint-rules#no-vacuous-absence-assertion): такой тест
проходит и когда поток выдал пустой список, и когда не выдал ничего, а `expectNoEmission` — это та
половина, которая падает, когда что-то приходит.

## `await` не опционален {#the-await-is-not-optional}

Каждый хелпер подписывается в момент вызова и завершается через промис, поэтому вызов, которого
никто не дождался, — это подписка, которую никто не закрыл, и проверка, которой не было. Раньше
такое уезжало в **следующий** тест: холодный HTTP-поток или `router.events` продолжал питать его,
пока сторожевой таймер не отклонял промис — необработанный и приписанный тесту, который тут ни при
чём.

Поэтому каждое ожидание, всё ещё открытое к концу теста, закрывается силой
...
нечему. Эта строка и есть рецепт: дождитесь вызова через `await` — или сохраните его в переменную и
дождитесь её до конца теста.
...
Эти хелперы подписываются **как подписчик**, а не как обычный наблюдатель, поэтому подписку можно
закрыть изнутри той эмиссии, которой ожидание разрешается. Именно так поступает `firstValueFrom`, и
именно это заставляет оба синхронных случая работать как надо:

```ts
const seen = vi.fn();

await expect(expectEmission(from([1, 2, 3, 4, 5]).pipe(tap(seen)))).resolves.toBe(1);
expect(seen).toHaveBeenCalledTimes(1); // один, а не пять

await expect(expectEmission(of(1).pipe(repeat()))).resolves.toBe(1); // бесконечный источник, разрешён
```

Если хелперам дать обычного наблюдателя, rxjs оборачивает его и возвращает подписку, только когда
`subscribe` отработал, — а для `of`, `from`, `range` и всего с `repeat()` это происходит после того,
как произведена вся последовательность. Тогда собиралось всё, что источник эмитил, хотя просили
одно значение, выполнялся каждый побочный эффект после принятого, а бесконечный синхронный источник
крутился внутри `subscribe`, куда сторожевому таймеру не добраться.

Поэтому спай `tap`, `finalize` или `defer` **не** вызывается для значений после принятого. Спека,
написанная под старое поведение, — пять вызовов `tap` там, где хелпер просил первое значение, —
теперь видит один, и это ровно то число вызовов, которое сделал бы продакшен-подписчик.

`expectEmissions(source$, 3)` берёт ровно три и на этом останавливается, а `expectNoEmission`
разрешается первой же эмиссией, а не концом последовательности.
...
Захвата `setTimeout` на моменте импорта достаточно для `vi.useFakeTimers()`, который подменяет
глобал потом. Для zone.js — нет: он подменяет его ещё пока грузится — задолго до этого пакета, — и
его подмена выбирает планировщик из `Zone.current` в момент **вызова**. Поэтому сторож раньше
попадал в виртуальную очередь внутри `fakeAsync`, где `tick(1_500)` в сторону `debounceTime(2_000)`
отклонял ровно тот поток, который крутил.

zone.js хранит нетронутую функцию под `__zone_symbol__setTimeout` — её-то сторож и берёт, когда
зона загружена. Так что фраза выше теперь верна и под зоной: ожидание — настоящее время, и ни один
`tick()`, написанный спекой, его не истечёт. Без zone.js ничего не меняется.
