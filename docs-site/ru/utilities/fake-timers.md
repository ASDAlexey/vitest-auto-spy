...
Она вычерпывает очередь целиком, а не пару фиксированных витков по ней. Иначе цепочка
`.then().then().then()`, которую начал таймерный колбэк, оставалась недоезжающей на одно звено, а
таймер, _поставленный_ из продолжения промиса, не выполнялся вообще — это как раз форма `delay()`,
retry и опросов, ради которой хелпер и существует.
...
упасть глубже с `timers are not mocked`. Так же она отказывается, когда сфейкованы только **часы**:
`mockSystemTime()` ставит один `Date`, и продвижение здесь нечего двигать, — раньше вызов молча
проходил, ничего не сделав. Ошибка называет `setupFakeTimers()` как то, что нужно тесту, который
двигает таймеры.
...
[`mockSystemTime(time)` и `withSystemTime(time, body)`](./event-loop) замораживают часы независимо
от того, запущены ли уже фейковые таймеры: с установленными фейками они двигают фейковые часы, без
них ставят фейки только на `Date` и оставляют таймеры настоящими.

В обоих случаях часы потом возвращаются на место — теперь и под фейками, которые поставила сама
сюита, где откат раньше был no-op'ом и обещание `withSystemTime` восстановить состояние «включая
падение» ни до чего не доходило. Время, которое спека прокрутила внутри блока, сохраняется, а не
выбрасывается: часы возвращаются туда, где они оказались бы, не выстави их блок, а не на мгновение
открытия блока.

::: warning `countStrayTimers()` слепа под фейковыми таймерами
`vi.useFakeTimers()` кладёт поверх обёрток [забытых таймеров](./setup) собственный `setTimeout`,
так что каждый таймер, который раздают фейковые часы, проходит мимо трекинга.
`expect(countStrayTimers()).toBe(0)` не говорит о файле, идущем на замороженных часах, ровным
счётом ничего, а `strayTimers` не компонуется ни с `globalFakeTimers`, ни с этим хелпером.
`vi.getTimerCount()` — очередь самих фейковых часов.
:::
