...
- **Вызванный без `new`, он бросает по имени,** с файлом и строкой вызова, потерявшего `new`. Иначе
  единственный способ узнать, что дубль использовали неправильно, — `TypeError` несколькими кадрами
  вглубь чужого кода.
- **Фабрика обязана вернуть объект.** JavaScript выбрасывает примитив, возвращённый из `new`, поэтому
  ...
  этого о таком сообщается сразу, вместе с исправлением: вернуть экземпляр из фабрики.
