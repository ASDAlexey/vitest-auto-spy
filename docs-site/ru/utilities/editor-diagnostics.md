...
description: Подчёркивать антипаттерны vitest-auto-spy прямо во время написания спеки — тридцать поставляемых ESLint-правил, которые WebStorm и другие IDE от JetBrains показывают нативно, а VS Code, Cursor и Windsurf — через расширение ESLint.
---
...
[`vitest-auto-spy/eslint-plugin`](/ru/utilities/eslint-plugin). Тридцать правил по настоящему
...
поэтому правила загораются **прямо в коде, без установки плагина** — те же тридцать проверок, в
...
бы держать вторую копию тридцати правил — на Kotlin — в согласии с TypeScript-овыми. Там, где путь через
...
и flat config выше уже достаточно: те же тридцать правил, в коде и в панели Problems.
