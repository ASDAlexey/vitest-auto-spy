...
**Границы.** Спека, которая следит за консолью, чтобы видеть напечатанное, *и* хочет этот вывод в логе,
...
```ts
import { consoleErrorSpy } from 'vitest-auto-spy/console'; // ❌ поставлен тем файлом, который импортировал первым
```
