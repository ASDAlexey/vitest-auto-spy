...
description: По разделу на каждое из двадцати пяти правил — что оно сообщает, на чём принимает решение, зачем оно в recommended, где сообщает о работающем коде и почему у него именно такая severity.
---
...
## Двадцать пять правил {#the-twenty-five-rules}
...
- объект, который `useValue` провайдера отдаёт в DI, написан ли он в слоте или в одном имени от него
  ...
  дубль учат отключать оба. Шаг по имени важен потому, что то правило идёт по имени *внутрь* слота:
...
- класс с непустым **`implements`**, который отстать *не может*: добавьте член в тип, и заглушка
...
- класс **без собственного имени** — классовое выражение в слоте свойства. Такой подменяет *экспорт*
...
потому, что это была *самая большая* оставшаяся семья в сюите, где все `error`-правила `recommended`
...
них *означает* дубль всего объекта — вопрос [`no-mocked-for-spy`](#no-mocked-for-spy), и здесь он не
...
[`prefer-provide-auto-spy`](#prefer-provide-auto-spy) идёт по имени *внутрь* `useValue`, а это правило
...
Собственная тема правила — сервис *без* DI, так что вычет делает именно то, о чём правило говорит;
...
**Стоит проверить, где правило вообще *не работает*, прежде чем решать, что оно слепо.** Один шард
...
сообщение советует `createAutoMock<T>()` — верный ответ для дубля *без* DI и неверный здесь. В
...
**Severity.** `warn` — одна из трёх градуированных severity, и единственная, градуированная по *виду*
...
| `Mock<T>`, `MockInstance<T>`, `MockedFunction<T>`, `MockedFunctionDeep<T>`, `PartialMock<T>` | `T extends Procedure \| Constructable` | не в отчёт — `T` здесь *функциональный* тип, поэтому ни один из них не может назвать класс в принципе; каждый типизирует один `vi.fn()`, и это верное использование |
...
*члены* собранного руками объектного типа, и в отчёт стоит брать объектный тип вокруг них. Это
