...
**`--ignore` for a finding the repository has already answered.** Some findings are right in
general and wrong for one repository in a way `doctor` cannot see: `angular-build-splitting-off` on
a project that patches the builder's code splitting back on, or `no-agent-instructions` in CI for a
repository that keeps its agent instruction files out of git — they exist on every developer's
...
its `error` line plus the indented lines under it still collects all of it. The report ends in the
tally whatever happened, a clean run included.
...
`truncated`), `exitCode`, `tally` (`errors`, `warnings`, `notes`) and every finding with
`check`, `severity`, `file`, `message`, `fix` and `details`. `--min-severity` shapes only the
text; the document always carries every finding. Fields only ever grow — one that changes meaning or
goes away raises `schema`. The exit code is the same as the text run's.

**`--format markdown` for a merge request note or a job summary.** The same document rendered as
GitHub/GitLab-flavoured markdown: a findings table (severity, check, file, message, fix) and the
tally, and for `perf` the phase table, the `slowestFiles` rows and the gate's verdicts as well. It is
built from the JSON document, so the two cannot disagree about a run; `|` and line breaks inside a
cell are escaped. Append it to `$GITHUB_STEP_SUMMARY`, or post it as a note.
...
which delegates to it — there is usually no configuration to edit: the builder reads a Vitest config
only when its target names one in `runnerConfig`. It takes the reporter and the file filter as
options instead, so the whole recipe is one command, and `perf` prints it with your project and target
filled in when it refuses a bare run:
...
The single quotes matter: `$VITEST_AUTO_SPY_PERF_REPORTER` has to reach the shell `perf` starts, not
the one you type in. The file paths the report carries are the spec sources, so the gate and
`--why` read the same way as under plain Vitest.
...
runs that are not measuring anything. Declared permanently, it has a name of its own —
`reporters: ['default', 'vitest-auto-spy/perf-reporter']`, or `--reporters=vitest-auto-spy/perf-reporter`
on a builder target — rather than a path into `node_modules/vitest-auto-spy/dist/`; a CI job then sets
...
At the bottom of the report, after the findings, `perf` states the budgets once — a test body, and
the three a file's bodies are held to, with what `--max-file-tests` is worth in this run — and then
prints what would fail `--gate` and nothing else: **files over budget**, with `time`, the
`budget` it is over, `over` (the two divided), `tests`, `ms/test`, `×median` and, when
`--baseline` names a committed baseline, `vs base` — how many times its recorded share of the run
the file takes now, or `new` — and **test bodies over budget**, the ones at or over
`--max-test-ms`. Each title counts every row over budget and says when `--top` left some off. The
budget flags draw the tables with or without `--gate`, so a report run and a gated run of the same
suite agree about what is over. The rows come from the same
...
The numbers come first, right-aligned, and the path last and whole, so an 80-column CI log still
...
Under `--gate`, the confirmation pass is followed by **the verdict table**: one row per candidate
with its `kind` (`test`, `file`, `grew`, `run`), the `first` reading, the one taken `again` on
its own, the `budget` and the `verdict` — `confirmed`, `not reproduced`, `unconfirmed` (nothing
could re-measure it), `single reading` (`--no-confirm`) or `over budget` for a whole-run budget.
The card under a confirmed finding compares the candidate against **its own** budget; it used to
show the file-total budget for a slow body or a baseline regression too, which printed "0.1× over"
over a finding that had just failed the run. The whole report ends in one tally, for the advice,
the gate and the baseline together.

`--format json` prints the same run as one document on stdout — `run` (files, tests, wall and CPU
milliseconds, the two medians, the phases, and `slowestFiles` — the `--top` slowest files, 10 by
default, each with its repository-relative path, total milliseconds, test count and per-phase
milliseconds), `budgets`, `gate` (`status`, `confirmation` and the
...
What the block says about the repository comes from a scan, not from a template. The rxjs line names
the setup file the tests actually run — the first `setupFiles` entry of an `@angular/build:unit-test`
or `@nx/angular:unit-test` target (`nx.json` `targetDefaults` included) when there is one, the Vitest
config's otherwise, where `sequence.setupFiles: 'list'` is an order and not a file. An Angular
repository that imports from `/angular/diagnostics`, `/angular/doubles` or `/angular/matchers` — or
still imports a name that moved there in 5.21.0 from `/angular` — gets one more line naming them.
...
| `--only <paths>`  | `init`   | Touch only these targets, comma-separated; a directory selects what `init` writes under it. `--only CLAUDE.md,.claude` keeps the block out of a tracked `AGENTS.md` and `GEMINI.md` in a repository that ignores only its Claude files. `--check` and `--uninstall` honour it |
