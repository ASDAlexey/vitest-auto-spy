...
description: Underline the vitest-auto-spy anti-patterns while the spec is being written — the twenty-eight shipped ESLint rules, shown natively by WebStorm and the other JetBrains IDEs, and by the ESLint extension in VS Code, Cursor and Windsurf.
---
...
[`vitest-auto-spy/eslint-plugin`](/utilities/eslint-plugin). Twenty-eight rules over a real syntax tree,
...
natively, so the rules light up **inline, with no plugin to install** — the same twenty-eight checks, in
...
then have to keep a second copy of twenty-eight rules — in Kotlin — in step with the TypeScript ones.
...
and the flat config above is enough: the same twenty-eight rules, inline and in the Problems panel.
