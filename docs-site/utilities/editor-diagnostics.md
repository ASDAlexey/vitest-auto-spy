...
description: Underline the vitest-auto-spy anti-patterns while the spec is being written — the thirty-six shipped ESLint rules, shown natively by WebStorm and the other JetBrains IDEs, and by the ESLint extension in VS Code, Cursor and Windsurf.
---
...
[`vitest-auto-spy/eslint-plugin`](/utilities/eslint-plugin). Thirty-six rules over a real syntax tree,
...
natively, so the rules light up **inline, with no plugin to install** — the same thirty-six checks, in
...
then have to keep a second copy of thirty-six rules — in Kotlin — in step with the TypeScript ones.
...
and the flat config above is enough: the same thirty-six rules, inline and in the Problems panel.
