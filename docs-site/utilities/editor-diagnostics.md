...
[`vitest-auto-spy/eslint-plugin`](/utilities/eslint-plugin). Twenty-three rules over a real syntax tree,
...
natively, so the rules light up **inline, with no plugin to install** — the same twenty-three checks, in
...
then have to keep a second copy of twenty-three rules — in Kotlin — in step with the TypeScript ones.
...
and the flat config above is enough: the same twenty-three rules, inline and in the Problems panel.
