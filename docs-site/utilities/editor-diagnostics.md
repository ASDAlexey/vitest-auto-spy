...
description: Underline the vitest-auto-spy anti-patterns while the spec is being written — the thirty-seven shipped ESLint rules, shown natively by WebStorm and the other JetBrains IDEs, and by the ESLint extension in VS Code, Cursor and Windsurf.
---
...
[`vitest-auto-spy/eslint-plugin`](/utilities/eslint-plugin). Thirty-seven rules over a real syntax tree,
...
natively, so the rules light up **inline, with no plugin to install** — the same thirty-seven checks, in
...
then have to keep a second copy of thirty-seven rules — in Kotlin — in step with the TypeScript ones.
...
and the flat config above is enough: the same thirty-seven rules, inline and in the Problems panel.
