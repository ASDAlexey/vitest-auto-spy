...
- `npm run check` is the gate — `deps:check`, typecheck, lint, `format:check`, jscpd, `llms:check`,
  ...
  tests, the type-instantiation budget (`types:budget`), the shared-env, zone, happy-dom, invariants,
  node, Bun, Bun-Angular and Rstest suites, the Bun isolate and built-preload lanes, then `build`
  with the dist smoke, size and cold-import checks after it. A change is not finished until it
  passes.
