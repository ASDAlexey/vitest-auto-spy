...
| one method of the object under test, recorded and still running                                   | `spyOnOwnMethod(sut, 'method')` — the `onlyMethodsToSpyOn` + `passthrough` packed; the drop-in for a bare banned `vi.spyOn`                                                       |
