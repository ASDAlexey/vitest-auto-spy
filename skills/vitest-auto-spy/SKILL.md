...
…just one method of it?         → spyOnOwnMethod(obj, 'method') — recorded, still calls through
…a native void method?          → spyOnVoidMethod(event, 'preventDefault') — strict-safe
```
...
| one method of the object under test, recorded and still running                                     | `spyOnOwnMethod(sut, 'method')` — the `onlyMethodsToSpyOn` + `passthrough` packed; the drop-in for a bare banned `vi.spyOn`; `prefer-spy-on-own-method` (`warn`, `--fix`) finds the long spelling |
| a native void method a handler is meant to call (`preventDefault`, `focus`)                         | `spyOnVoidMethod(event, 'preventDefault')` — records the call and seeds `undefined`, so `strict` does not throw on it                                                                             |
...
| `NG0303` / `NG0304` / silence from a directive spec                                                 | `createDirectiveHost({ template, scope: [Module] })`; Angular ≥ 22.2, no host markup needed: `TestBed.createDirective(Dir, { tagName, bindings })`, which `stable` takes                          |
...
| `let deps: T & Spy<T>` spelt out for an `autoMocked<T>()` assigned in `beforeEach`                  | `let deps: AutoMocked<T>`                                                                                                                                                                         |
...
**After any `eslint --fix` over specs, run `npx tsc --noEmit`.** The forty-nine rules in
...
**`no-hand-assigned-global` reports a double assigned straight to a global** — `global.fetch = vi.fn(…)`, `window.matchMedia = vi.fn()`, `window.localStorage = { getItem: vi.fn() }` — unless the file puts it back in `afterEach` / `afterAll` / `onTestFinished`. Replace the assignment with `mockValueProp(globalThis, 'fetch', vi.fn(…))` (undone by `restoreMockedProps()`, which `setupAutoSpy()` runs), use `blockNetwork()` when the spec only has to stay offline, `stubWebStorage('localStorage')` for the storages, and `stubWorker({ respond })` for `globalThis.Worker`. Do not "fix" it by adding a restore as the last line of the `it`: that form is reported too, because a red assertion skips it. The same rule reports any value written into an imported object — `environment.production = true` — and `--fix` rewrites it to `mockValueProp(environment, 'production', true)` inside a test or `beforeEach`.
