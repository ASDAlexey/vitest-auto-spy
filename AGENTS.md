...
`TypeError` (§17). Nothing else in the field does this — `vi.mockObject` is Vitest-only,
...
on a live `Node` or another `EventTarget`, the call now reports this itself, before patching
...
`expect(el.getBoundingClientRect).toHaveBeenCalled()`, and the return value is still callable as the
undo. Both go through `mockValueProp`, so `restoreMockedProps()` (and `setupAutoSpy()` between tests)
