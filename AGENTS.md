...
| `no-redundant-mock-reset`         | `error` | `--fix` / suggest | a mock reset in a `beforeEach` / `afterEach` / `beforeAll` / `afterAll` — `vi.clearAllMocks()` / `resetAllMocks()` / `restoreAllMocks()`, `x.mockClear()` / `mockReset()` / `mockRestore()` — the runner already performs between tests → delete it, and the hook with it where that is all it held. **Silent** until the flags are known: options `{ clearMocks, restoreMocks, mockReset }` first, then a `vitest.config.*` / `vite.config.*` searched upwards from the file and read as text. Flag matches call, never family — `restoreMocks` walks only the spies `vi.spyOn` installed. `--fix` only as the first statement of the file's only `beforeEach`; a suggestion otherwise; never inside a test body                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `selfReturning names 'x', which is not a spied method`                                                                                              | the name is not a method of the double — a typo, or a restricting `onlyMethodsToSpyOn` left it out; on a type-driven double, `then` and `constructor` are never spies                                                                                                       | fix the name or the whitelist; the value is silently never returned otherwise (§5)                                                                                                                                                                                                                                                                                                                                                                                                                                |
