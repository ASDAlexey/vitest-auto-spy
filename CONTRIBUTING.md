...
| `npm run check`            | Every step the CI `test`, `static` and `bun` jobs run, on one Node, in waves (`scripts/check.mjs`): all static checks at once, then the suites side by side, then `build` and the checks that read `dist/`, then the invariants alone. The first failure stops the rest and prints that stage's output; `-- --serial` runs one stage at a time, `-- --verbose` prints passing stages too, `-- --no-cache` skips the local prettier / tsc / ESLint caches in `.cache/check/` |
...
| Agent reference   | `AGENTS.md` — a map kept under 32 KB (a spec in `src/cli/init.spec.ts` fails past it) — plus one `agent-docs/*.md` per section; a new section goes into `agent-docs/` with a stub in the map; `npm run agents:sync` formats them and regenerates the stubs' outlines | —                            |
...
| Per-entry size    | `size-entries.json`, when an entry grew — `npm run size:entries:update`; growth over 3 % since the last release also needs a CHANGELOG line naming the entry                                                                                                         | `npm run size:entries:check` |
| Module graph      | `cold-import.json`, when an entry pulled in more modules — `npm run cold-import:update`; the same CHANGELOG rule applies                                                                                                                                             | `npm run cold-import:check`  |
...
**Size is a decision, not a refresh.** `size-entries.json` and `cold-import.json` keep the numbers
of the last release in a `released` block that only the `version` script moves. Re-recording with
`npm run size:entries:update` / `cold-import:update` keeps the record honest, but an entry more than
3 % above `released` — or a module more in its graph — fails the check until the `## [Unreleased]`
section has a line that names the entry (`` `/angular` ``, `vitest-auto-spy/angular`, "the root
entry", or "every entry") and says what the bytes buy.

**A new lint rule is a module of its own** in `src/lib/eslint/`, named after the rule, plus one line
in the registry in `rules.ts`; the registry defines no rule itself.
...
3. Run `npm run check` locally — it runs every step of CI's `test`, `static` and `bun` jobs in
   parallel waves, and it builds at the end. Locally it caches prettier, `tsc` and ESLint results
   in `.cache/check/`; ESLint's cache cannot see a type change in a file another file depends on, so
   CI lints without it — run `npm run check -- --no-cache` when a change moves a shared type.
   CI also repeats the `test` job on Node 22, 24 and 26 and runs two jobs
