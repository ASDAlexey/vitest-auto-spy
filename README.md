...
- 📏 Lint rules and one-line test-run hygiene — forty-eight rules in `vitest-auto-spy/eslint-plugin` (five `--fix`, eighteen suggestions, four of them for a suite mid-migration off jasmine), `setupAutoSpy()` — with `preset: 'strict'` for every guard at its strictest grade, and under `isolate: false` `strayListeners` / `restoreGlobals` for the listeners and raw global assignments a file leaves the next one, `restoreStorageSpies` (on by default) for the storage spy happy-dom will not let `mockRestore()` remove
...
| `vitest-auto-spy/dom-stubs`           | the globals a component builds for itself — `stubIntersectionObserver`, `stubResizeObserver`, `stubMutationObserver`, `stubObserver`, `stubMediaElement`, `stubAbortController`, the entry builders, `stubWebStorage` with its `snapshot()`, and `stubWorker`. On the root entry until 4.0.0                                                                                                                                                                              | —                                                        |   ✅   |
