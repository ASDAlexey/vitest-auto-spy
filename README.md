...
- 📏 Lint rules and one-line test-run hygiene — thirty-seven rules in `vitest-auto-spy/eslint-plugin` (three `--fix`, twelve suggestions, four of them for a suite mid-migration off jasmine), `setupAutoSpy()` — with `preset: 'strict'` for every guard at its strictest grade
...
**Three of the thirty-seven fix on their own, twelve offer suggestions**, and the split is not about how hard
...
all run ESLint natively, so the thirty-seven rules appear inline, in the **Problems** tool window, and
...
and then keep a second copy of thirty-seven rules, in Kotlin, in step with the TypeScript ones.
