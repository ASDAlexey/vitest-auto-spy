...
[![minzipped size](https://img.shields.io/badge/minzip-20.3%20kB-brightgreen)](#install)
...
- 📏 Lint rules and one-line test-run hygiene — thirty-eight rules in `vitest-auto-spy/eslint-plugin` (three `--fix`, thirteen suggestions, four of them for a suite mid-migration off jasmine), `setupAutoSpy()` — with `preset: 'strict'` for every guard at its strictest grade
...
the whole core behave identically to the Vitest entry. The rest of `/angular` is not routed to Bun:
the matcher registrars (`registerSignalMatchers`, `registerDirectiveMatchers`,
`registerResourceMatchers`) need the runner's `expect.extend`, the TestBed diagnostics its
suite-level hooks, and the overrides, `extendWithAutoSpies`, `provideAutoSpyForToken`, the stub
factories and the prop, platform and dialog doubles are simply not exported there.
...
| `enableAngularDiagnostics(opts?)` / `assertNoPendingRequests()`                                             | `/angular`                    | Dead NgModule imports, dead `schemas`, an unspied provider, a provider the component's own `providers` shadows and unflushed HTTP requests, as failures ([details](#diagnostics--five-silent-failures-made-loud))                                                                                                                                       |
...
**Four of the thirty-eight fix on their own, thirteen offer suggestions**, and the split is not about how hard
...
all run ESLint natively, so the thirty-eight rules appear inline, in the **Problems** tool window, and
...
and then keep a second copy of thirty-eight rules, in Kotlin, in step with the TypeScript ones.
