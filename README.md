...
- 🧱 The providers every suite hand-rolls, provided once — `provideRouterDouble({ url })` for a `Router` that derives `url`, `routerState` and `events` from one URL rather than four guesses; `provideWindowDouble(WINDOW, { screen })` and `provideDocumentDouble({ … })`, which merge what the spec names **over** the real jsdom object, so the member nobody thought of still answers; `provideMatDialogData(MAT_DIALOG_DATA, data)` and `provideMatDialogRef(MatDialogRef)` for the Material dialog trio, with Material itself staying out of this package's dependencies
...
- 🏗️ Doubles for what the code builds itself — `mockConstructor` / `stubConstructor` for `new`, plus `stubMediaElement`, `stubAbortController`, `stubWebStorage` and the observer stubs
...
- 📏 Lint rules and one-line test-run hygiene — thirty-six rules in `vitest-auto-spy/eslint-plugin` (three `--fix`, eleven suggestions, four of them for a suite mid-migration off jasmine), `setupAutoSpy()` — with `preset: 'strict'` for every guard at its strictest grade
...
| `vitest-auto-spy/angular`        | `provideAutoSpy`, `injectSpy`, `renderShallow`, `setInputs`, `createWithAutoSpies`, `stable`/`flushEffects`, `settleResource`, `mockResourceProp`, the `mock*Prop` helpers, `trackRecomputations` / `trackEffectRuns`, the `window` / `document` and Material-dialog doubles, signal & resource matchers, TestBed diagnostics | `@angular/core`, `@angular/platform-browser`             |   ✅   |
...
| `provideRouterDouble(init?)` / `injectRouterDouble()`                                    | `/angular-router`             | A `Router` derived from one URL — real `serializeUrl` / `createUrlTree`, `navigate` spies, `emitNavigation()` over a `BehaviorSubject`                                                                |
...
**Three of the thirty-six fix on their own, eleven offer suggestions**, and the split is not about how hard
...
all run ESLint natively, so the thirty-six rules appear inline, in the **Problems** tool window, and
...
and then keep a second copy of thirty-six rules, in Kotlin, in step with the TypeScript ones.
...
| `provideRouterDouble(init?)` / `injectRouterDouble(injector?)` / `createRouterDouble(init?)` _(`/angular-router`)_                          | A `Router` derived from one URL — `navigate` spies, `setUrl`, `emitNavigation`, and the router's own URL serialization                                                                                                                                                        |
...
| `provideMatDialogData(token, data)` / `provideMatDialogRef(Ref, init?)` / `injectMatDialogRef(Ref)` _(Angular)_                             | The Material dialog trio — the token and the ref class are arguments, so `@angular/material` stays out of this package                                                                                                                                                        |
