...
- 📏 Lint rules and one-line test-run hygiene — forty-eight rules in `vitest-auto-spy/eslint-plugin` (five `--fix`, eighteen suggestions, four of them for a suite mid-migration off jasmine), `setupAutoSpy()` — with `preset: 'strict'` for every guard at its strictest grade
...
| `no-redundant-mock-reset`         |   `error`   | `--fix` / suggest | a mock reset **inside a hook** — `vi.clearAllMocks()` / `resetAllMocks()` / `restoreAllMocks()`, `x.mockClear()` / `mockReset()` / `mockRestore()` — that the runner already performs between tests → delete it. **Silent until** the flags are known: `['error', { clearMocks: true, restoreMocks: true, mockReset: true }]`, or a `vitest.config.*` / `vite.config.*` found above the file. The flag has to match the call: `restoreMocks` alone does not make a `clearAllMocks()` dead, since it never reaches a plain `vi.fn()`. `--fix` only where nothing can have run in between — the first statement of the file's only `beforeEach` — a suggestion everywhere else, and **never** in a test body                                                                                    |
...
**Five of the forty-eight fix on their own, eighteen offer suggestions**, and the split is not about how hard
...
all run ESLint natively, so the forty-eight rules appear inline, in the **Problems** tool window, and
...
and then keep a second copy of forty-eight rules, in Kotlin, in step with the TypeScript ones.
